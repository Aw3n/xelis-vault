...
## ✅ Core Contracts Status (2026-07-28)

| Contract | Address | Status | Tests |
|----------|---------|--------|-------|
| **PriceOracle v2.1** | `764ad585c2f484e54ea9dd06a7fb8b81397ba2487d37298f27edce3747d836dd` | ✅ Live | propose→execute→distribute_reward full pipeline |
...
## ✅ v5 Fresh Deploy (2026-07-27/28) — Miner + Governance + Oracle Rewards

| Contract | Address | Status |
|----------|---------|--------|
| **VLTToken v5.1** (pub fn mint_to) | `7275c55d711789b1b746cd4695b04c0e393a0db74ecf72360c5544b73368cfab` | ✅ Deployed |
| **VLT Asset v5.1** | `2de72ed3ea2d8ff30e6df57ba3a4d993dedfa8636d207d43d09e33615bfde2c6` | ✅ Created |
| **XelisVaultMiner v2.1** (pub fn distribute_reward) | `21ed1297c7ed4001a4a7c9a4bb89b10da0b0f3ad0312545a5af4a761200af207` | ✅ Configured (vc, va, tr set), 1 miner registered, PriceOracle registered as service |
| **PriceOracle v2.1** (calls distribute_reward at entry 18) | `764ad585c2f484e54ea9dd06a7fb8b81397ba2487d37298f27edce3747d836dd` | ✅ Live, full pipeline tested |
...
## Status (2026-07-28) — PriceOracle v2.1 + distribute_reward Full Pipeline 🎉

### ✅ Major Milestone: Oracle → Miner → VLT Reward Pipeline

The complete cross-contract reward distribution flow is now working end-to-end:
1. **PriceOracle v2.1** `execute_price` calls `Contract::call(18, [caller, 1, true], {})` on the miner
2. **XelisVaultMiner v2.1** `pub fn distribute_reward` (chunk 18) validates miner, calculates dynamic reward, calls `Contract::call(4, [miner_addr, reward], {})` on VLTToken
3. **VLTToken v5.1** `pub fn mint_to` (chunk 4, changed from `entry`) mints VLT and transfers to the miner's address

**Result**: 71,347,030 VLT (0.71 VLT) minted to admin wallet in first successful distribution ✅

### 🔑 Key Fixes

| Fix | File | Change |
|-----|------|--------|
| VLTToken.mint_to must be `pub fn` | `contracts/token/VLTToken.slx:83` | `entry` → `pub fn` (cross-contract `Contract::call()` requires `Access::All`) |
| Added entry wrapper for wallet calls | `contracts/token/VLTToken.slx:374` | `entry mint_to_entry(to, amount)` → delegates to `pub fn mint_to` |
| distribute_reward call from oracle | `contracts/oracle/PriceOracle.slx:54` | Calls entry 18 (was 8 — bug from copy-paste) |
| MINTER_PREFIX is `"mi_"` not `"m_"` | `contracts/token/VLTToken.slx:21` | Storage key for minter whitelist uses `"mi_"` prefix |

### ✅ New Deployments (2026-07-28)

| Contract | Address | Notes |
|----------|---------|-------|
| **VLTToken v5.1** | `7275c55d711789b1b746cd4695b04c0e393a0db74ecf72360c5544b73368cfab` | `pub fn mint_to` at chunk 4, `entry mint_to_entry` at chunk 27 |
| **VLT Asset v5.1** | `2de72ed3ea2d8ff30e6df57ba3a4d993dedfa8636d207d43d09e33615bfde2c6` | Created from new VLTToken |
| **XelisVaultMiner v2.1** | `21ed1297c7ed4001a4a7c9a4bb89b10da0b0f3ad0312545a5af4a761200af207` | Replaced v2 (`fd370918...`), `pub fn distribute_reward` at chunk 18 |
| **PriceOracle v2.1** | `764ad585c2f484e54ea9dd06a7fb8b81397ba2487d37298f27edce3747d836dd` | Calls entry 18 on miner, `mc` set to new miner ✅ |

### ✅ Remaining Contracts — Same Addresses
- **xUSD**: `909576c1fcd889ec443b63a4ce014bf756fcb8afd74c8c0ee902cac03384e3fc`
- **xUSD Asset**: `d8bd79a2aa33ad4a6fa0ac2b2440515124445ecce0468e070a8a09bb5ea9442f`
- **VaultEngine**: `667b165c8c9cd6cc3464378799e38b172e0f2e912f4b5c6202d37a8da3939bcc`
- **PSM v5.1**: `9f2667447b9a850ba4b260c19cd2c3786bc4a3c5559a08332a9e13bfa47191ae`
- **VaultSwapV2**: `1b6699398e2acecbdd1fd372952696cfc37b99eb1dcac45a7216661f96c60422`

### ✅ Pipeline Test Results
- Propose price (entry 2): ✅ `result: 0` at topo 292850
- Execute price (entry 3) after 3-block timelock: ✅ `result: 0` at topo 292858+
- distribute_reward called successfully: ✅ `pub fn` at chunk 18 accessed via `Contract::call(18, ...)`
- VLT minted to miner: ✅ 71,347,030 atomic VLT (0.71 VLT) transferred to admin/miner wallet
- pp cleared after execution: ✅
- Storage key `mi_<miner_hash> = true` confirmed: ✅ (MINTER_PREFIX is `"mi_"`)

### 🔑 Lessons Learned
1. **VLTToken's `set_minter` stores at `mi_<hash>`** (not `m_<hash>`): `MINTER_PREFIX = "mi_"` at line 21
2. **First `execute_price` succeeded silently** only because no miner was registered — `distribute_reward` returned early at line 538 (`miner_opt.is_none()`). The real `Contract::call()` validation happened only when a miner existed and the function tried to mint VLT.
3. **`entry` functions cannot be called via `Contract::call()`**: All cross-contract callable functions must be `pub fn`. The error "Chunk is not public" means the target chunk has `Access::Entry`, not `Access::All`.
4. **Nested `Contract::call()` chains work**: PriceOracle → Miner (entry 18) → VLTToken (entry 4) — each link requires the target to be `pub fn`.
