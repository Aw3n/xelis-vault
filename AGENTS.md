...
```
/private/tmp/xelis-blockchain/target/release/xelis_wallet \
  ...
  --daemon-address https://testnet-node.xelis.io --network testnet \
...
An off-chain bot that:
1. Fetches XEL/USD price from CoinGecko (or MEXC as fallback)
2. Proposes + executes price on the oracle contract
3. Runs on a configurable block interval (default: every 100 blocks)

Usage:
```bash
ORACLE_HASH=<oracle_contract_hash> python3 price_bot.py
```

The bot skips updates if price changed <1% from current on-chain price.
